...
export class SecureAPIClient {
  ...
  // ============= DASHBOARD API =============
  /**
   * Get the revenue summary for one of the signed-in client's properties.
   * Omit the period for all-time totals.
   *
   * The client is identified by the backend from the auth token only. This used
   * to send an 'X-Simulated-Tenant' header, which lets the browser claim to be
   * any client, so it has been removed.
   */
  async getDashboardSummary(
    ...
    if (period) {
      queryParams.append("month", String(period.month));
      queryParams.append("year", String(period.year));
    }
  ...
  /**
   * Properties owned by the signed-in client, used to build the property selector.
   */
  async getDashboardProperties(): Promise<
